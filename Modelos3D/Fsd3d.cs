...
public partial class Fsd3d : Node3D
{
	...
	public override void _Ready()
	{
		...
		AtivarMover = false;
*/
		RecursoPontoPathDesgin.Instantiate<PontoPathDesgin>();
	}
	...
	public List<Vector2> ObterPontos(Vector2 ini, Vector2 fim, double angOffSetDeg = 0)
	{
		...
		for (double i = angIni; i < lim; i += 1)
		{
			...
			float x = (float)Math.Cos(ang);//- 0.5f;
			float y = 1f + (float)Math.Sin(ang);//+ 0.5F;//+ 1 - 0.5
			...
			saida.Add(Meth.RotatePoint(new Vector2(
				x, y
			), -angOffSetDeg) + ini);//angOffSetDeg * 2.
	...
	public List<Vector2> ObterPontosTotais()
	{
		...
		Vector2 PosIniAnt = Instrucoes[0].PInicial;
		double angAnt = 0;
