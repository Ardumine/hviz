...
{
    public static Vector2[] InterpolateXY(Vector2[] poses, int count)
    {
        double[] xs = new double[poses.Length];
        double[] ys = new double[poses.Length];
        ...
        (double[] xsSaida, double[] ysSaida) = InterpolateXY(xs, ys, count);

        var saida = new Vector2[xsSaida.Length];

        for (int i = 0; i < xsSaida.Length; i++)
        {
            saida[i] = new Vector2((float)xsSaida[i], (float)ysSaida[i]);
        }
...
public static class CubicSplineAnt
{
    /// <summary>
    /// Generate a smooth (interpolated) curve that follows the path of the given X/Y points
    /// </summary>
